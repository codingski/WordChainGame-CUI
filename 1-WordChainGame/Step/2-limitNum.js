...
// 글자 수 제한
const number = 2;
const controlWords = mappedWords.filter((word) => word.length === number);
const wordList = controlWords;
...
while(1) {
    ...
    let filteredWords = wordList.filter((word) => word[0] === firstWord[firstWord.length -1])
